...
//#ifdef CONJUGATE_HEAT
  int     group_id;         /* group id indicting solid or fluid           */
//#endif
...
public:
  ...
  virtual void Grad_z_h(int nz, int ntot, double *HL, double *EL) {}
  virtual void Grad_z_d(double *EL) {}
  virtual void Set_state(char type);
  virtual void H_Refine(int * to_split, int nfields, Bndry **Ubc, int *flag);  
  virtual void Trans(int ntot, double *HL, double *EL, Nek_Trans_Type ntt){}
  virtual void FFT(int ntot, double *HL, double *EL, Nek_Trans_Type ntt){}
...
class Bndry{
 public:
  ...
  double   *w;
  //zwang 08032017
  double   aux_data[2];         // auxiliary data storage for Robin Boundary
};
