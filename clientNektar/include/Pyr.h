...
public:
  // Constructors
  Pyr();                                             // default constructor
  Pyr(Element *);                                    // constructor
  Pyr(int i_d, char ty, int *qsize, int *jsize, Coord *X);
  Pyr(int i_d, char ty, int L, int Qa, int Qb, int Qc, Coord *X);

  // Memory routines
  void Mem_J(int *list, char Pyrp);
  void Mem_Q();
  ...
  void MassMat (LocMat *);                           // return mass-maPyr(ces)
  ...
  void Trans(Element *, Nek_Trans_Type);              // Transform to Pyr
  void Iprod(Element *);                              // Inner product to Pyr
  void Iprod_d (Element *,Basis *, Basis *);          // Inner product to Pyr
  void Iprod_3d(Element *,Basis *, Basis *, Basis *); // Inner product to Pyr

  // Derivatives routines
  void Grad (Element *, Element *, Element *, char Pyrp);
  void Grad_d(double *, double *, double *, char Pyrp);  
  void Grad_h(double *, double *, double *, double *, char Pyrp); 
  ...
  // Error functions
  void Set_field(char *sPyrng);                      // set field to function
  void Error(char *sPyrng);                          // compare with function
  double L2_error_elmt(char *sPyrng);
  double H1_error_elmt(char *sPyrng);
  void Verror(double *u, char *sPyrng);
  double Int_error( char *sPyrng);
  double Norm_li();
  double Norm_l2();
  ...
  int get_face_q1(int faceid){
    ...
    case 4:
      return qb;
    default:
      fprintf(stderr,"Pyr:get_face_q1: Unknown faceid");
      exit(1);
      return 0;
    }
  }
  ...
  int get_face_q2(int faceid){
    ...
    case 4:
      return qc;
    default:
      fprintf(stderr,"Pyr:get_face_q2: Unknown faceid");
      exit(1);
      return 0;
    }
  }
