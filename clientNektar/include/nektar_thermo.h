...
#ifdef SPM
  Element_List  *CONCENTR;
  double **ust;                  /* multistep storage */
  double **vst;
  double **wst;
  double **pst;
...
#endif
#ifdef TURBULENCE
  Element_List  *TurbVis;
  double        ReffTurbVis;
  double  *FilterVector;       /* Stored filter constant for LES     */
#endif
...
#ifdef THERMO
Bndry        *ReadBCs_T(FILE *fp, Element *U);
Bndry        *ReadMeshBCs_T(FILE *fp, Element_List *Mesh);
#endif
...
#ifdef TURBULENCE
void ConvPlusTurb(Domain *omega);
#endif
...
void SetP_pBCs(Domain *omega);
static void CalcP_pbc(Bndry *B, Element_List *V[3]);
