...
void InitBLAC(Bsystem *Bsys){
  ...
  get_proc_grid(pllinfo.nprocs, &i, &j);
  BLACS_PARAMS[3] = i;
  BLACS_PARAMS[4] = j;
...
void  GatherScatterIvert(Bsystem *Bsys){
  ...
  nnz_per_rank = ivector(0,pllinfo.nprocs-1);
  izero(pllinfo.nprocs,nnz_per_rank,1);
  nnz_per_rank[pllinfo.procid] = SM[0].nz;

  temp_nnz_per_rank = ivector(0,pllinfo.nprocs-1);
  izero(pllinfo.nprocs,temp_nnz_per_rank,1);

  gisum (nnz_per_rank,pllinfo.nprocs,temp_nnz_per_rank);
  ...
  for (i = 1; i < pllinfo.nprocs; i++){
    send_buf_Index = ivector(0,SM[0].nz-1);
    send_buf_Jndex = ivector(0,SM[0].nz-1);
    send_buf_value = dvector(0,SM[0].nz-1);

    memcpy (send_buf_Index,SM[0].IA,SM[0].nz*sizeof(int));
    memcpy (send_buf_Jndex,SM[0].JA,SM[0].nz*sizeof(int));
    memcpy (send_buf_value,SM[0].AA,SM[0].nz*sizeof(double));

    partner_to_send_data = pllinfo.procid+1;
    partner_to_receive_data =  pllinfo.procid-1;
    if (partner_to_send_data == pllinfo.nprocs)
      partner_to_send_data = 0;
    if (partner_to_receive_data == -1)
      partner_to_receive_data = pllinfo.nprocs-1;

    /* trace number of SM transfers in order to get nz value right */
    index = pllinfo.procid-i;
    if (index < 0)
      index = index+pllinfo.nprocs;

    j = SM[0].nz; //length of massege to be sent

    SM[0].deallocate_SMatrix();
    SM[0].allocate_SMatrix(nnz_per_rank[index],1,1);

    if (pllinfo.procid == 0){
      mpi_isend (send_buf_Index,j,partner_to_send_data ,0);
      mpi_irecv (SM[0].IA,SM[0].nz,partner_to_receive_data,0);
    }
    else{
      mpi_irecv (SM[0].IA,SM[0].nz,partner_to_receive_data,0);
      mpi_isend (send_buf_Index,j,partner_to_send_data ,0);
    ...
    if (pllinfo.procid == 0){
      mpi_isend (send_buf_Jndex,j,partner_to_send_data ,1);
      mpi_irecv (SM[0].JA,SM[0].nz,partner_to_receive_data,1);
    }
    else{
      mpi_irecv (SM[0].JA,SM[0].nz,partner_to_receive_data,1);
      mpi_isend (send_buf_Jndex,j,partner_to_send_data ,1);
    ...
    if (pllinfo.procid == 0){
      ...
      mpi_drecv (SM[0].AA,SM[0].nz,partner_to_receive_data,2);
    }
    else{
      mpi_drecv (SM[0].AA,SM[0].nz,partner_to_receive_data,2);
      mpi_dsend (send_buf_value,j,partner_to_send_data ,2);
    ...
    free(send_buf_Index);free(send_buf_Jndex);free(send_buf_value);
