...
void Quad::update_bndry(Bndry *Bc,int save){
 ...
 if(save){
  ...
  vector_def("x y",Bc->bstring);
  /* ce107 changes begin */
//  double coordx = vert[Quad_vnum[Bc->face][1]].x + dparam("ORIGINX"),
//         coordy = vert[Quad_vnum[Bc->face][1]].y + dparam("ORIGINY");
//zwang
  double coordx = vert[vnum(Bc->face,1)].x + dparam("ORIGINX"),
         coordy = vert[vnum(Bc->face,1)].y + dparam("ORIGINY");
  /* set top vertex */
  vector_set(1,&(vert[vnum(Bc->face,1)].x),&(vert[vnum(Bc->face,1)].y),
	     Bc->bvert+1);
  GetFaceCoord(Bc->face,&X);
