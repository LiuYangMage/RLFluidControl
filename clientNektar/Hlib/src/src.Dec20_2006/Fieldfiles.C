...
#if defined(__sgi) || defined(cm5) || defined(_AIX) || defined (__hpux) || defined (__sparc) || defined (_CRAYMPP) || defined(__APPLE__)
#define ieeeb
#endif
...
void ReadDataNormal(FILE *fp, Field *f){
  ...
  DO_PARALLEL{ /* read in relevant elements */
    ...
    if(hybridformat){
      for(cnt = 0,i = 0; i < pllinfo.nloop; ++i)
	cnt += gnfacet[pllinfo.eloop[i]];
      
      /* copy in relevant data from gsize to f->size */
      lsize   = f->size   = ivector(0,cnt-1);
      lnfacet = f->nfacet = ivector(0,pllinfo.nloop-1);
      
      for(i = 0,cnt = 0,n=0; i < f->nel; ++i)
	if((n < pllinfo.nloop)&&(i == pllinfo.eloop[n])){
	  lnfacet[n] = gnfacet[i];
    ...
    ntot = data_len(pllinfo.nloop,f->size,f->nfacet,f->dim);
	...
	for(i = 0; i < f->nel; ++i){
	  ...
	  if((eid < pllinfo.nloop)&&(i == pllinfo.eloop[eid])){
...
void ReadDataShuffled(FILE *fp, Field *f){
  ...
  DO_PARALLEL{ /* just read in relevant elements of shuffled data   */
    ...
    if(hybridformat){
      ...
      for(i = 0,cnt = 0; i < pllinfo.nloop; ++i)
	cnt += gnfacet[shuf[pllinfo.eloop[i]]];
      ...
      for(i = 0; i < pllinfo.nloop; ++i){
	lnfacet[i] = gnfacet[shuf[pllinfo.eloop[i]]];
	cnt = isum(shuf[pllinfo.eloop[i]],gnfacet,1);
	...
	lskip = 11;
      ...
      for(i = 0; i < pllinfo.nloop; ++i){
	icopy(lskip,gsize + shuf[pllinfo.eloop[i]]*lskip,1,
	      f->size + i*lskip,1);
	lnfacet[i] = lskip;
      }
    }
    ...
    if(f->nz <=1)
      ...
      if(option("SurForce")) {
        ...
        if(option("Sur_dump")){
         ...
         for(i=0; i < pBCinfo.nloop; ++i)
           fprintf(fp,"%d", pBCinfo.eloop[i]);
           fputc('\n',fp);
        }
        else{
          for(i = 0; i < pllinfo.nloop; ++i)
...
//additions for running sectional forces
        ...
        if(option("SurForce")){
          ...
          spBCinfo = (short *) calloc(pBCinfo.nloop,sizeof(short));
          for (n = 0; n < pBCinfo.nloop; n++)
            spBCinfo[n] = (short) pBCinfo.eloop[n];

          if(option("Sur_dump")){
            fwrite(spBCinfo, sizeof(short),  pBCinfo.nloop, fp);
            free(spBCinfo);
          }
          else{
            fwrite(spllinfo, sizeof(short),  pllinfo.nloop, fp);
    ...
    if(f->nz <= 1)
      DO_PARALLEL{
       ...
       if(option("SurForce")){
        if(option("Sur_dump"))
       fwrite(pBCinfo.eloop, sizeof(int),  pBCinfo.nloop, fp);
        else
       fwrite(pllinfo.eloop, sizeof(int),  pllinfo.nloop, fp);
      }
     else
