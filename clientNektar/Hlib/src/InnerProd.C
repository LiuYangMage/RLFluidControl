...
void Hex::HelmHoltz(Metric *lambda){
  ...
  //form_diprod(u1,u2,u3,NULL);
  if(lambda->v) dvmul(qt,lambda->v,1,*h_3d[0],1,*h_3d[0], 1); 
  if(lambda){
   ...
   if ( (dparam("SVVfactor") ? 1:0) && type != 'p') {
    register int i,j,k,cnt;
    int    fl = face[0].l, el;
    double Qb,*filter = dvector(0,Nm-1);
    static double  Epsilon;
...
//test
ROOTONLY
  fprintf(stdout,"\n SVV:Epsilon=%lf, NCutMT=%d, SVVfactor=%lf, lmax=%d,NCutM =%d before\n",Epsilon,NCutMT,SVVfactor,lmax,NCutM);
