...
void set_con_info(Element_List *U, int nel, int *partition){
  ...
  elmtid  = ivector(0,Max_Nedges*pllinfo.nloop-1);
  edgeid  = ivector(0,Max_Nedges*pllinfo.nloop-1);
  edgegid = ivector(0,Max_Nedges*pllinfo.nloop-1);
  
  for(n = 0; n < pllinfo.nprocs; ++n){
    if(n == pllinfo.procid) continue;
    
    ifill(Max_Nedges*pllinfo.nloop,-1,elmtid,1);
    ifill(Max_Nedges*pllinfo.nloop,-1,edgeid,1);
    ifill(Max_Nedges*pllinfo.nloop,-1,edgegid,1);
    
    /* gather together information about edges on this process 
       connecting with processor n */
    if(U->fhead->dim() == 2){
      for(i = 0,nedges=0; i < pllinfo.nloop; ++i){
	E = U->flist[pllinfo.eloop[i]];
  ...
  cinfo = (ConInfo *)malloc(pllinfo.ncprocs*sizeof(ConInfo));
  for(cnt=0,c = pllinfo.cinfo; c; c = c->next, ++cnt)
    memcpy(cinfo + cnt, c,sizeof(ConInfo));
  
  c = pllinfo.cinfo;
  pllinfo.cinfo = cinfo;

  for(cnt=0; cnt < pllinfo.ncprocs-1; ++cnt)
    pllinfo.cinfo[cnt].next = pllinfo.cinfo + cnt+1;
...
void set_pllinfo(Element_List *EL, char *name){
  ...
  pllinfo.nprocs = numnodes();
  pllinfo.procid = mynode();
  pllinfo.gnel   = nel;
  ...
  DO_PARALLEL{
    ...
    sprintf(fname, "%s.part.%d",name,pllinfo.nprocs); 
...
#ifdef PARALLEL
#ifdef _BLACS
    pllinfo.AdjacentPartitions = test_partition_connectivity(EL,partition,&i);
    pllinfo.NAdjacentPartitions = i;
#endif
#endif
    ...
    for(i = 0; i < pllinfo.nloop; ++i){
      for(id = j = i; j < pllinfo.nloop; ++j)
	id = (pllinfo.eloop[j] < pllinfo.eloop[id])? j:id;
      eid = pllinfo.eloop[id];
      for(j = id; j > i; --j)
	pllinfo.eloop[j] = pllinfo.eloop[j-1];
      pllinfo.eloop[i] = eid;
    }
    
    //  pllinfo.etypes  = ivector(0, nel-1);
    pllinfo.efacets = ivector(0, nel-1);
    pllinfo.eedges  = ivector(0, nel-1);
    pllinfo.efaces  = ivector(0, nel-1);
      
    for(i=0;i<nel;++i){
      //    pllinfo.etypes[i]  = (int) EL->flist[i]->identify();
      pllinfo.eedges[i] = EL->flist[i]->Nedges;
      pllinfo.efaces[i] = EL->flist[i]->Nfaces;
    
      pllinfo.efacets[i] = EL->flist[i]->Nedges + EL->flist[i]->Nfaces;
      if(EL->flist[i]->dim() == 3) ++pllinfo.efacets[i]; /* interior */
    }
    
    // cumfacets[i] gives the offset from zero of the first 
    // facet of element i
    pllinfo.cumfacets = ivector(0, nel);
    pllinfo.cumfacets[0] = 0;
    for(i=1;i<nel+1;++i)
      pllinfo.cumfacets[i] = pllinfo.cumfacets[i-1]+pllinfo.efacets[i-1];
    ...
    pllinfo.partition = partition; // save for later usage (Particulary LE).
  }
  else{
    /* set up standard ordering  */
    pllinfo.nloop = nel;
    pllinfo.eloop = ivector(0,nel-1);
    for(i=0; i < nel; ++i)
      pllinfo.eloop[i] = i;
  }
}
...
void set_pBCinfo(Element_List *EL,Bndry *Ubc, char *name){
  ...
  pBCinfo.nprocs = numnodes();
  pBCinfo.procid = mynode();
  pBCinfo.gnel   = n_elem;
  ...
  DO_PARALLEL{
    ...
    id=0;
    pBCinfo.eloop = ivector(0, pBCinfo.nloop);

    for(i = 0; i < nel; ++i){
       if(part[i]>-1)
       pBCinfo.eloop[id++]=part[i];
    }

  }
  else{
    /* set up standard ordering  */
    pBCinfo.nloop = n_elem;
    pBCinfo.eloop = ivector(0,n_elem-1);
    for(i=0; i < n_elem; ++i)
      pBCinfo.eloop[i] = i;
  }
}
...
Element_List *Part_elmt_list(Element_List *GEL, char trip){
  ...
  if(GEL->fhead->dim() == 2){
	...
	if(vb = GEL->flist[pllinfo.eloop[k]]->vert[i].base){
	...
	if(vb = GEL->flist[pllinfo.eloop[k]]->vert[i].base){
      ...
      for(i = 0; i < new_E[k]->Nedges; ++i){
	if(base = GEL->flist[pllinfo.eloop[k]]->edge[i].base){
  ...
  for(E=EL->fhead;E;E=E->next){
    ...
    E->set_geofac();
    if(E->geom->singular)
      fprintf(stderr,"Jacobian is negative in element %d\n",E->id+1);
  }
...
static void  add_pf(int elmt, int side){
  ...
  for(i = 0; i < pllinfo.npface; ++i)
    if((elmt == pllinfo.npfeid[i])&&(side == pllinfo.npfside[i]))
      flag = 0;

  if(flag){
    pllinfo.npface++;
    pllinfo.npfeid  = (int *)realloc(pllinfo.npfeid,
				     pllinfo.npface*sizeof(int));
    pllinfo.npfeid[pllinfo.npface-1] = elmt;
    pllinfo.npfside = (int *) realloc(pllinfo.npfside,
				      pllinfo.npface*sizeof(int));
    pllinfo.npfside[pllinfo.npface-1] = side;
  }
}
